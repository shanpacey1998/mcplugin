...
$regex = sprintf(
    '/^(?!.*(%s|\.git|\.gitea|phpchecks|references))(?=.*\.(php|ya?ml)$).*$/i',
    preg_quote($basePath . '/build', '/')
);

echo sprintf('Regex used for inclusion/exclusion: %s', $regex)  . PHP_EOL;
...
foreach ($regexIterator as $file) {
    echo sprintf('Found file: %s', $file) . PHP_EOL;
}
...
echo sprintf('Done in %.3fs', microtime(true) - $start) . PHP_EOL;